...
class DateHandler:
    @staticmethod
    def sanitize_for_mysql(
        date: Union[str, datetime, int,float, None]
    ) ->Optional[str]:
        """
        ...
        if isinstance(date, str):
            ...
            parsed = dateparser.parse(
                ...
                settings={"PREFER_DATES_FROM": "past",
                          ...
                          "RELATIVE_BASE": datetime(int(extracted_year), 1, 1) if extracted_year else datetime.now()
                }
            )
            if parsed is None:
                return None
    ...
    def parse_periodo(period:Union[str, dict, tuple, None]) -> Dict[str, Optional[str]]:
        ...
        if isinstance(period, str):
            ...
            patterns = [
                ...
                lambda m: (
                    dateparser.parse(m.group(1), languages=['pt', 'de', 'en'], settings={"DATE_ORDER": "DMY"}), 
                    dateparser.parse(m.group(2), languages=['pt', 'de', 'en'], settings={"DATE_ORDER": "DMY"})
                )),
                # Dias
                (r'^hoje$',                        lambda m: (hoje, hoje)),
                (r'^ontem$',                       lambda m: (hoje - timedelta(days=1),
                ...
                (r'[úu]ltimos?\s+(\d+)\s+dias?',   lambda m: (hoje - timedelta(days=int(m.group(1))),
                                                            hoje)),
                (r'[úu]ltimos?\s+(\d+)\s+meses',   lambda m: PeriodHandler._ultimos_meses(int(m.group(1)))),

                # Meses específicos (português) - CORRIGIDO para aceitar "dezembro 2025" e "dezembro de 2025"
                (r'dezembro\s+(?:de\s+)?(\d{4})', lambda m: PeriodHandler._mes_especifico(12, m.group(1))),
                ...
                #(r'novembro\s+(?:de\s+)?(\d{4})', lambda m: PeriodHandler._mes_especifico(11, m.group(1))),
                #(r'novembro$', lambda m: PeriodHandler._mes_especifico(11, None)),
                (r'outubro\s+(?:de\s+)?(\d{4})', lambda m: PeriodHandler._mes_especifico(10, m.group(1))),
                (r'outubro$', lambda m: PeriodHandler._mes_especifico(10, None)),
                (r'setembro\s+(?:de\s+)?(\d{4})', lambda m: PeriodHandler._mes_especifico(9, m.group(1))),
                (r'setembro$', lambda m: PeriodHandler._mes_especifico(9, None)),
                (r'agosto\s+(?:de\s+)?(\d{4})', lambda m: PeriodHandler._mes_especifico(8, m.group(1))),
                (r'agosto$', lambda m: PeriodHandler._mes_especifico(8, None)),
                (r'julho\s+(?:de\s+)?(\d{4})', lambda m: PeriodHandler._mes_especifico(7, m.group(1))),
                (r'julho$', lambda m: PeriodHandler._mes_especifico(7, None)),
                (r'junho\s+(?:de\s+)?(\d{4})', lambda m: PeriodHandler._mes_especifico(6, m.group(1))),
                (r'junho$', lambda m: PeriodHandler._mes_especifico(6, None)),
                (r'maio\s+(?:de\s+)?(\d{4})', lambda m: PeriodHandler._mes_especifico(5, m.group(1))),
                (r'maio$', lambda m: PeriodHandler._mes_especifico(5, None)),
                (r'abril\s+(?:de\s+)?(\d{4})', lambda m: PeriodHandler._mes_especifico(4, m.group(1))),
                (r'abril$', lambda m: PeriodHandler._mes_especifico(4, None)),
                (r'mar[cç]o\s+(?:de\s+)?(\d{4})', lambda m: PeriodHandler._mes_especifico(3, m.group(1))),
                (r'mar[cç]o$', lambda m: PeriodHandler._mes_especifico(3, None)),
                (r'fevereiro\s+(?:de\s+)?(\d{4})', lambda m: PeriodHandler._mes_especifico(2, m.group(1))),
                (r'fevereiro$', lambda m: PeriodHandler._mes_especifico(2, None)),
                (r'janeiro\s+(?:de\s+)?(\d{4})', lambda m: PeriodHandler._mes_especifico(1, m.group(1))),
                (r'janeiro$', lambda m: PeriodHandler._mes_especifico(1, None)),

                # Trimestre (pt)
                (r'q(\d)|(\d)º\s+trimestre',       lambda m: PeriodHandler._trimestre(int(m.group(1) or m.group(2)))),

                # ========= ALEMÃO =========
                # Dias
                (r'^heute$',                       lambda m: (hoje, hoje)),
                (r'^gestern$',                     lambda m: (hoje - timedelta(days=1),
                                                            hoje - timedelta(days=1))),

                # Semana
                (r'diese woche',                   lambda m: PeriodHandler._semana_atual()),
                (r'letzte woche',                  lambda m: PeriodHandler._semana_atual()),  # se quiser tratar igual
                ...
                # Últimos N dias / meses
                (r'letzten?\s+(\d+)\s+tage?',      lambda m: (hoje - timedelta(days=int(m.group(1))),
                                                            hoje)),
                (r'letzten?\s+(\d+)\s+monate?',    lambda m: PeriodHandler._ultimos_meses(int(m.group(1)))),
                ...
                (r'(?:märz|maerz)\s+(\d{4})', lambda m: PeriodHandler._mes_especifico(3, m.group(1))),
                ...
                (r'^yesterday$',                   lambda m: (hoje - timedelta(days=1),
                                                            hoje - timedelta(days=1))),
                ...
                (r'last\s+(\d+)\s+days?',          lambda m: (hoje - timedelta(days=int(m.group(1))),
                                                            hoje)),
                (r'last\s+(\d+)\s+months?',        lambda m: PeriodHandler._ultimos_meses(int(m.group(1)))),
            ...
            meses_conhecidos = ['janeiro', 'fevereiro', 'março', 'marco', 'abril', 'maio', 'junho', 
                              'julho', 'agosto', 'setembro', 'outubro', 'novembro', 'dezembro',
                              'januar', 'februar', 'märz', 'maerz', 'mai', 'juni', 'juli', 
